 ...
 * с использованием AI анализа содержимого
 */
...
Верни ТОЛЬКО slug категории без дополнительного текста.`;
  ...
  if (geminiClient) {
    try {
      ...
      if (result.response.promptFeedback?.blockReason) {
        console.error('  └─ ⚠️  Контент заблокирован Gemini:', result.response.promptFeedback.blockReason);
      ...
      console.error('  └─ ⚠️  Gemini ошибка:', error instanceof Error ? error.message : String(error));
    }
  }
  ...
  if (!aiResponse && openRouterKey) {
    try {
      console.log('  └─ Используем OpenRouter API...');
      ...
      console.error('  └─ ⚠️  OpenRouter ошибка:', error instanceof Error ? error.message : String(error));
    }
  }
...
async function main() {
  ...
  try {
    // Проверка API ключей
    if (!geminiClient && !openRouterKey) {
      console.error('❌ Ошибка: Не настроены API ключи для AI!');
      console.error('   Установите GEMINI_API_KEY или OPENROUTER_API_KEY в .env файле');
      process.exit(1);
    }

    console.log('✓ AI сервис настроен:', geminiClient ? 'Gemini' : 'OpenRouter');
